  ...
  toString(strings: TemplateStringsArray | string, ...values: StrinfigyValue[]) {
    if (this.isTemplateStringsArray(strings)) {
      return strings.reduce(
        (prev, current, index) => `${prev}${current}${this.stringifier.strinfigy(values[index])}`,
        ""
      );
    }
    ...
    if (typeof strings === "string" && values.length) {
      console.warn(
        "It is not recommended to use logger as a function, instead use logger as template tag with ``, example: logger.log`My msessage`"
      );
